...
5. Orchestrator runs: Market → News → Sentiment → Preprocess → Prophet → OpenAI/Groq LLM → Report
...
- **Problem:** When `OPENAI_API_KEY`, `GROQ_API_KEY`, or `NEWS_API_KEY` is missing, Python printed to stdout and exited. Node expected only JSON on stdout, so `JSON.parse(stdout)` failed with a generic error.
- **Fix:** Config validation now writes to stderr. Agent runner now includes both stdout and stderr in error messages. Set `LLM_PROVIDER=openai` (recommended) or `groq` to select your LLM provider.
...
| **LLM_PROVIDER**          | Set to `openai` (recommended) or `groq`       |
| **OPENAI/GROQ API Key**   | Required based on LLM_PROVIDER selection      |
| **NEWS_API_KEY**          | Required in `.env`                            |
| **Python deps**           | `pip install -r apps/agents/requirements.txt` |
| **Run from project root** | `npm run dev` from project root               |
...
- **If you see "Configuration error"** → Check `.env` for LLM_PROVIDER, OPENAI_API_KEY (or GROQ_API_KEY), and NEWS_API_KEY.
