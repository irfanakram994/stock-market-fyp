...
export async function POST(request: NextRequest) {
  try {
    ...
    const email = authUser.data.user?.email || session.superAdmin.email;
    const authUserId = authUser.data.user?.id;
    if (!authUserId || !email) {
      return NextResponse.json({ success: false, error: "Unable to resolve authenticated super admin." }, { status: 401 });
    }
